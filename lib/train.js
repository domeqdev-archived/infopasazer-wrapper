...
let getTrain = async (id) => {
    ...
    let final = data.map((x) => {
        ...
        let planned_arrival = new Date(`${data.split(".").reverse().join("-")} ${przyjazd} +0200`).getTime();
        let planned_departure = new Date(`${data.split(".").reverse().join("-")} ${odjazd} +0200`).getTime();

        return {
            station: stacja,
            arrival: planned_arrival ? {
                planned: planned_arrival,
                real: new Date(planned_arrival + arrival_delay * 1000).getTime(),
                delay: arrival_delay
            } : null,
            departure: planned_departure ? {
                planned: planned_departure,
                real: new Date(planned_departure + departure_delay * 1000).getTime(),
